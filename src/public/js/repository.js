...
function autoHeight(element) {
    if($(window).height() < $(document).height()) {
        $(element).css('height', $(document).height() - 65 + 'px');
    } else {
        if($(window).height() > 400)
            $(element).css('height', $(window).height() - 65 +'px'); // on soustrait la hauteur de l'écran à la hauteur du menu
        else
...
$(function() {
    // redimensionnement d'éléments afin qu'ils occupent au minimum la taille de l'écran actuel, et que cela se fasse une fois puis toute les X secondes
/*    autoHeight('.dashboard .menu');
    ...
    $(document).on('submit', '.action-add', function (e) {
        ...
        $.ajax({
            ...
            if(response.status == "success") {
                let dataReplace = '<div class="list"><div class="pull-right"><button id="edit" data-attribute="'+ response.id +'"f class="btn btn-xs btn-warning live"><span class="glyphicon glyphicon-edit" aria-hidden="true"></button></div><a href="/action/' + response.id + '"><li>' + value + '</li></a></div>';
    ...
    $(document).on('submit', '.action-edit', function (e) {
        ...
        $.ajax({
            ...
            if(response.status == "success") {
                ...
                let dataReplace = '<div class="list"><div class="pull-right"><button id="edit" class="btn btn-xs btn-warning live" data-attribute="'+ id +'"><span class="glyphicon glyphicon-edit" aria-hidden="true"></button></div><a href="' + id + '"><li>' + escapeHtml(value) + '</li></a></div>';
    ...
    $(document).on('submit', '.action-edit-date-creation', function (e) {
        ...
        $.ajax({
            ...
            console.log(response.status);
            if(response.status == "success") {
                let dataReplace = '<div class="inner action-info action-date-creation"><div class="pull-right"><button id="edit-date-creation" class="btn btn-xs btn-warning live"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></button></div>'+ value +'</div>';
    ...
    $(document).on('submit', '.action-edit-date-realisation', function (e) {
        ...
        $.ajax({
            ...
            console.log(response.status);
            if(response.status == "success") {
                let dataReplace = '<div class="inner action-info action-date-realisation"><div class="pull-right"><button id="edit-date-realisation" class="btn btn-xs btn-warning live"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></button></div>'+ value +'</div>';
    ...
    $(document).on('submit', '.action-edit-date-butoir', function (e) {
        ...
        $.ajax({
            ...
            if(response.status == "success") {
                let dataReplace = '<div class="inner action-info action-date-butoir"><div class="pull-right"><button id="edit-date-butoir" class="btn btn-xs btn-warning live"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></button></div>'+ value +'</div>';
